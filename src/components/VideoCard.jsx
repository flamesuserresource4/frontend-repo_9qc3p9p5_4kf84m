...
const VideoCard = ({ module }) => {
  const { title, educator, duration, views, category } = module;
  const color = colorPairs[module.id % colorPairs.length];

  return (
    <div className="group cursor-pointer">
      <div className={`relative aspect-video w-full overflow-hidden rounded-xl bg-gradient-to-tr ${color[0]} ${color[1]} shadow-sm`}>
        <div className="absolute inset-0 bg-black/10 group-hover:bg-black/0 transition-colors" />
        <PlayCircle className="absolute inset-0 m-auto text-white/90 group-hover:scale-105 transition-transform" size={56} />
